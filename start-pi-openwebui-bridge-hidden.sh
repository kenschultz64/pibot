...
echo "Starting Pi OpenWebUI Bridge in background..."
echo "Base URL: ${PI_PUBLIC_BASE_URL}/v1"
echo "Workspace: $PI_WORKSPACE"
echo "Logs: $BRIDGE_DIR/bridge.out.log and $BRIDGE_DIR/bridge.err.log"

nohup node src/server.js > bridge.out.log 2> bridge.err.log &
echo $! > bridge.pid
echo "PID: $(cat bridge.pid)"
